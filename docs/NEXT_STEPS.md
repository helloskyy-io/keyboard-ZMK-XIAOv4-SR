...
- **Left** = scroll. **Right** = pointer. **Experiment #2 result:** Right trackball still **totally non-functional** (force-awake commented out on both). So our code almost certainly did **not** cause it – **sensor almost certainly failed completely.** Left (scroll): testing whether it goes to sleep after leaving it alone longer (1–2 min so far, no delay/weirdness like before).
- **Right trackball history:** Right was flaky and not very accurate; user cleaned only the sensor eye (plastic bubble) with a q-tip and 90% rubbing alcohol. It started failing worse after that, then after a few flashes stopped working entirely. (Could be coincidence, or moisture/alcohol affecting the sensor or board; swap test then showed the issue follows the right half, not the sensor.)
- **Swap result (hardware test):** User swapped the sensor modules left↔right (wiring goes with sensor, so wiring ruled out). **After swap: left scroll still worked, right mouse still broken.** So the issue **follows the right half**, not the physical sensor. The problem is **right-half controller or software** (right MCU, right firmware/SPI/config, or right-side input-split/listener path).
- **Trying a new right-side controller:** The Seeed Studio boards (e.g. Seeed XIAO BLE) only need **firmware** – no separate board prep. You can try a **new** same-model controller on the right half: install it on the right (replace the current right MCU), flash the **right** firmware to it, and test. If the pointer works, the old right MCU was likely bad; if it still doesn’t, the issue may be elsewhere (socket, design). Otherwise, sending the board back to the builder for repair is an option if you prefer not to debug further.

- **Swapping controller boards (brave test):** You can swap the **MCU boards** left↔right, then reflash each for its new position. **Result: same thing – left TB works, right TB does not.** So we’ve now ruled out: **sensor** (swapped), **wiring** (swaps with sensor), **MCU** (swapped). The only thing that never moved is the **right half’s PCB** – the physical board, its trackball socket, and the traces from that socket to the MCU. So the fault is almost certainly on the **right PCB** (bad trace, cold joint, damaged socket, or something that got worse over time; cleaning could have been a red herring or moisture could have affected the right PCB’s trackball circuit). Left and right overlays use the same trackball pins (SPI1, gpio0 2 for CS, gpio0 29 for IRQ), so firmware/config is symmetric – not a pin-mismatch in code. **Next step:** Send the right half to the builder for repair (they can inspect/rework the right PCB’s trackball path), or inspect the right PCB yourself for cold joints, damaged traces near the trackball connector, or corrosion.

**Git check (did we break it with code?):** From commit `e6506a4` (builder fix, pin to v0.3) to now, the **only** changes that touch the right trackball are in **`skreecustom_right.overlay`**: (1) `force-awake` commented out, (2) `force-awake-4ms-mode` added as a comment, (3) line 1: `//` → ` //` (leading space). **No changes** to `skreecustom.dtsi` (listeners/split path). So we didn’t remove or change any right-side structure – only commented out force-awake and added a comment. **Last-resort test:** Revert the right overlay to the state at `e6506a4` (force-awake back on, no leading space), reflash **only the right** half, and see if the right trackball comes back (even flaky). If yes, something about the current right overlay is involved. If no, we’re still at right-PCB or split-path.
...
| **Trackballs** | **Diagnosis:** Sensor swap + controller swap (with reflash) both give same result: left TB works, right TB does not. So fault is **right half’s PCB** (socket/traces to trackball), not sensor, wiring, or MCU. Send right half for repair or inspect right PCB. |
